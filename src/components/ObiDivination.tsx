        ...
        <div className="min-h-screen bg-gradient-to-b from-[#0c0a07] via-[#050505] to-black text-ifa-text p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
            ...
            {(step === 'casting' || step === 'result') && (
                <div className="w-full max-w-md flex flex-col items-center z-10">
                    ...
                    {/* OBI CASTING TRAY — Premium Engraved Ebony Plate */}
                    <div
                        className="flex-shrink-0 aspect-square rounded-full relative flex items-center justify-center mb-8"
                        style={{
                            width: 'min(320px, 88vw)',
                            height: 'min(320px, 88vw)',
                            background: 'radial-gradient(ellipse at 32% 28%, #5c2e14 0%, #2a1008 45%, #130703 100%)',
                            boxShadow: '0 16px 48px rgba(0,0,0,0.9), 0 4px 12px rgba(0,0,0,0.6), inset 0 0 40px rgba(0,0,0,0.65)',
                            border: '2px solid rgba(100,55,20,0.35)'
                        }}
                    >
                        {/* Decorative outer groove ring */}
                        <div className="absolute inset-[6px] rounded-full pointer-events-none" style={{
                            border: '2.5px solid rgba(160,100,40,0.22)',
                            boxShadow: 'inset 0 0 16px rgba(0,0,0,0.5)'
                        }} />

                        {/* Wood grain texture */}
                        <div className="absolute inset-[14px] rounded-full opacity-[0.10] pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')]" />

                        {/* Inner cloth/raffia surface */}
                        <div className="absolute rounded-full pointer-events-none" style={{
                            inset: '14px',
                            background: 'radial-gradient(ellipse at 42% 35%, #5a3b1e 0%, #3b2210 55%, #1e0e06 100%)',
                            boxShadow: 'inset 0 0 45px rgba(0,0,0,0.8), inset 0 6px 18px rgba(0,0,0,0.5)'
                        }} />

                        {/* Iyerosun dust glow */}
                        <div className="absolute rounded-full pointer-events-none" style={{
                            inset: '22%',
                            background: 'radial-gradient(ellipse, rgba(210,170,85,0.1) 0%, transparent 75%)',
                            filter: 'blur(10px)'
                        }} />
                        ...
                        {step === 'casting' && !isAnimating && (
                            <div className="absolute inset-0 flex items-center justify-center bg-black/45 backdrop-blur-xs z-20 rounded-full">
                                <button 
                                    onClick={castObi}
