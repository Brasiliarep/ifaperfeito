...
import { Hand, Sparkles, Wind, ArrowDown, User, ArrowUp, CheckCircle2, Camera, Loader2, FastForward } from 'lucide-react';
        ...
        <div className="w-full flex flex-col items-center py-4 relative animate-fade-in">
            ...
            {onSetState && (
                <div className="mt-6 flex flex-col items-center gap-3">
                    <label className={`flex items-center gap-2 px-6 py-3 bg-ifa-surface border border-ifa-gold/30 hover:border-ifa-gold rounded-xl cursor-pointer text-sm font-bold uppercase tracking-wider text-ifa-gold transition-all hover:bg-ifa-gold/10 shadow-lg ${isAnalyzing ? 'opacity-70 pointer-events-none' : ''}`}>
                        {isAnalyzing ? (
                            <>
                                <Loader2 className="animate-spin text-ifa-gold" size={18} />
                                <span>Ifá analisando foto...</span>
                            </>
                        ) : (
                            <>
                                <Camera size={18} />
                                <span>Olho de Ifá (Analisar Foto)</span>
                            </>
                        )}
                        <input 
                            type="file" 
