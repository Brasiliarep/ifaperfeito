...
const TextReader: React.FC<TextReaderProps> = ({ text, label, large = false, forceLang }) => {
    ...
    const speakPart = (textPart: string, lang: string, rate: number, pitch: number, delay: number = 0): Promise<void> => {
        return new Promise<void>((resolve) => {
            setTimeout(() => {
                ...
                const activeLang = localStorage.getItem('ifa_language') || 'pt-BR';

                // Try to find a voice that matches the specific language requested
                const voices = window.speechSynthesis.getVoices();
                const matchingVoice = voices.find(v => v.lang.toLowerCase().startsWith(lang.toLowerCase().slice(0, 2)));
                
                if (selectedVoice && (lang === 'pt-BR' || lang === 'pt-PT' || !hasYorubaSupport)) {
                    utterance.voice = selectedVoice;
                } else if (matchingVoice) {
                    utterance.voice = matchingVoice;
                }

                if (lang === 'yo-NG' || lang === 'yo' || forceLang === 'yo-NG') {
                    // Ritual Mode: Deep and Slow (Thunder Voice Protocol)
                    utterance.rate = 0.75;
                    utterance.pitch = 0.7;
                    utterance.lang = hasYorubaSupport ? 'yo-NG' : activeLang;
                } else {
                    utterance.rate = 0.95;
                    utterance.pitch = 0.9;
                    utterance.lang = lang;
    ...
    const toggleSpeech = async (e: React.MouseEvent) => {
        ...
        const activeLang = forceLang || localStorage.getItem('ifa_language') || 'pt-BR';
        ...
        try {
            if (text.includes('Ipe:') || text.includes('Ofo:')) {
                ...
                if (ase) await speakPart(ase, activeLang, 0.9, 0.9, 1500);
            } else {
                const clean = text
                    .replace(/[*#_~`]/g, '')
                    .replace(/\[.*?\]\(.*?\)/g, '')
                    .replace(/\s+/g, ' ')
                    .trim();
                await speakPart(clean, activeLang, activeLang === 'yo-NG' || activeLang === 'yo' ? 0.7 : 1.0, activeLang === 'yo-NG' || activeLang === 'yo' ? 0.7 : 1.0);
            }
        } finally {
            isSpeakingRef.current = false;
            setIsPlayingUI(false);
