...
const InterpretationView: React.FC<Props> = ({ data, oduInfo, initialSelections, onReset, onEndSession, language, onOpenMandala, isStudent }) => {
    ...
    // Chat inline cumulativo
    interface ChatMessage { id: string; question: string; answer: string; shortSummary: string; ritualType: string; ritualTitle: string; basic: any; medium: any; complete: any; }
    const [chatHistory, setChatHistory] = useState<ChatMessage[]>([]);
    const [currentQuestion, setCurrentQuestion] = useState("");
    const [isAsking, setIsAsking] = useState(false);
    // Seleções e preços por mensagem do chat
    const [chatSelections, setChatSelections] = useState<Record<string, { level: string; price: number; saved: boolean }>>({});
    ...
    const handleAskQuestion = async (e?: React.FormEvent) => {
        ...
        setIsAsking(true);
        try {
            const result = await askSpecificQuestion(data.oduName, data, q, language);
            const msgId = crypto.randomUUID();
            setChatHistory(prev => [...prev, { id: msgId, question: q, answer: result.fullAnswer || "", shortSummary: result.shortSummary || "", ritualType: result.ritualType || "ebo", ritualTitle: result.ritualTitle || "", basic: result.basic, medium: result.medium, complete: result.complete }]);
            setChatSelections(prev => ({ ...prev, [msgId]: { level: 'basic', price: result.basic?.estimatedPrice || 50, saved: false } }));
        } catch (err) { console.error(err); }
        finally { setIsAsking(false); }
    };
    const closeChat = () => { }; // compat

    const RITUAL_TYPE_LABELS: Record<string, string> = { ebo: 'Ebó', akose: 'Akose', amuleto: 'Amuleto', ofo: 'Ofó', oriki: 'Oriki' };
    const RITUAL_TYPE_COLORS: Record<string, string> = { ebo: 'bg-amber-900/40 text-amber-300 border-amber-700', akose: 'bg-green-900/40 text-green-300 border-green-700', amuleto: 'bg-purple-900/40 text-purple-300 border-purple-700', ofo: 'bg-blue-900/40 text-blue-300 border-blue-700', oriki: 'bg-pink-900/40 text-pink-300 border-pink-700' };

    const RitualPanel = ({ msg }: { msg: ChatMessage }) => {
        const sel = chatSelections[msg.id] || { level: 'basic', price: 50, saved: false };
        const levels = [
            { id: 'basic', label: 'Simples', data: msg.basic, defaultPrice: msg.basic?.estimatedPrice || 50 },
            { id: 'medium', label: 'Médio', data: msg.medium, defaultPrice: msg.medium?.estimatedPrice || 150 },
            { id: 'complete', label: 'Completo', data: msg.complete, defaultPrice: msg.complete?.estimatedPrice || 350 },
            { id: 'dispensar', label: 'Dispensar', data: null, defaultPrice: 0 },
        ];
        const active = levels.find(l => l.id === sel.level);
        const activeData = active?.data;
        const typeLabel = RITUAL_TYPE_LABELS[msg.ritualType] || 'Ebó';
        const typeColor = RITUAL_TYPE_COLORS[msg.ritualType] || RITUAL_TYPE_COLORS.ebo;

        const handleSave = () => {
            if (!activeData || sel.level === 'dispensar') return;
            const newRemedy: CustomRemedy = {
                id: msg.id,
                category: msg.ritualType,
                problem: msg.question,
                solution: {
                    basic: msg.basic ? { ...msg.basic, title: msg.basic.description } : { description: '', instructions: '', ingredients: [] },
                    medium: msg.medium ? { ...msg.medium, title: msg.medium.description } : { description: '', instructions: '', ingredients: [] },
                    complete: msg.complete ? { ...msg.complete, title: msg.complete.description } : { description: '', instructions: '', ingredients: [] },
                },
                selection: sel.level as EboSelectionType,
            };
            setSelections(prev => ({ ...prev, customRemedies: [...prev.customRemedies.filter(r => r.id !== msg.id), newRemedy] }));
            setChatSelections(prev => ({ ...prev, [msg.id]: { ...sel, saved: true } }));
        };

        return (
            <div className="mt-4 bg-ifa-base-dark border border-ifa-border rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                    <span className={`text-xs font-bold px-2 py-1 rounded-full border ${typeColor}`}>{typeLabel}</span>
                    {msg.ritualTitle && <span className="text-xs text-ifa-neutral italic">{msg.ritualTitle}</span>}
                </div>
                {/* Tabs */}
                <div className="flex gap-2 mb-4 flex-wrap">
                    {levels.map(l => (
                        <button key={l.id} onClick={() => setChatSelections(prev => ({ ...prev, [msg.id]: { ...sel, level: l.id, price: l.defaultPrice, saved: false } }))}
                            className={`px-3 py-1 rounded text-xs font-bold uppercase transition-colors ${sel.level === l.id
                                ? l.id === 'dispensar' ? 'bg-gray-700 text-gray-200' : 'bg-ifa-gold text-ifa-base'
                                : 'bg-ifa-surface border border-ifa-border text-ifa-neutral hover:border-ifa-gold'
                                }`}>
                            {l.label}
                        </button>
                    ))}
                ...
                {sel.level !== 'dispensar' && activeData && (
                    <div className="space-y-3">
                        <h5 className="text-ifa-gold font-bold text-sm">{activeData.description}</h5>
                        {activeData.ingredients?.length > 0 && (
                            <div>
                                <p className="text-xs font-bold uppercase text-ifa-neutral mb-1">Materiais:</p>
                                <ul className="list-disc list-inside text-sm space-y-0.5">
                                    {activeData.ingredients.map((i: string, idx: number) => <li key={idx} className="text-ifa-text">{i}</li>)}
                                </ul>
                            </div>
                        )}
                        {activeData.instructions && (
                            <div>
                                <p className="text-xs font-bold uppercase text-ifa-neutral mb-1">Preparo:</p>
                                <p className="text-sm leading-relaxed whitespace-pre-line text-ifa-text">{activeData.instructions}</p>
                            </div>
                        )}
                        {activeData.ofo && (
                            <div className="bg-black/30 p-3 rounded border-l-4 border-ifa-gold">
                                <p className="text-xs uppercase font-bold text-ifa-gold mb-1">Ofó:</p>
                                <p className="italic text-sm">{activeData.ofo}</p>
                                {activeData.translation && <p className="text-xs text-ifa-neutral mt-1">{activeData.translation}</p>}
                            </div>
                        )}
                        {/* Orçamento + Salvar */}
                        <div className="flex items-center gap-3 pt-2 border-t border-ifa-border">
                            <span className="text-xs text-ifa-neutral">Orçamento (R$):</span>
                            <input type="number" value={sel.price}
                                onChange={e => setChatSelections(prev => ({ ...prev, [msg.id]: { ...sel, price: parseFloat(e.target.value) || 0 } }))}
                                className="w-20 bg-ifa-base border border-ifa-border text-white text-sm p-1 rounded text-right"
                            />
                            <button onClick={handleSave} className={`ml-auto px-4 py-1.5 rounded text-xs font-bold uppercase flex items-center gap-1 transition-colors ${sel.saved ? 'bg-green-700 text-white' : 'bg-ifa-wood text-white hover:opacity-90'}`}>
                                {sel.saved ? <><CheckCircle2 size={12} /> Salvo</> : <><FileText size={12} /> Salvar para Impressão</>}
                            </button>
                        </div>
                    </div>
                )}
                {sel.level === 'dispensar' && <p className="text-sm text-ifa-neutral italic">Solução ritual dispensada para esta questão.</p>}
            </div>
        );
    };
                        ...
                        <div className="animate-fade-in text-lg text-ifa-text leading-relaxed whitespace-pre-line font-light">
                                ...
                                <div className="space-y-4">
                                    <div className="bg-purple-900/20 border border-purple-700/50 p-4 rounded-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-purple-300 font-bold uppercase text-sm">Ayanmo e Ori (Destino e Cabeça Espiritual)</h4>
                                            <TextReader text={data.destinyAndOri} />
                                        </div>
                                        <p className="text-ifa-text leading-relaxed whitespace-pre-line">{data.destinyAndOri || "Cuide do seu Ori com oferendas e meditação."}</p>
                                    </div>
                                    <div className="bg-blue-900/20 border border-blue-700/50 p-4 rounded-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-blue-300 font-bold uppercase text-sm">Conselho Geral de Ifá</h4>
                                            <TextReader text={data.generalAdvice} />
                                        </div>
                                        <p className="text-ifa-text leading-relaxed whitespace-pre-line">{data.generalAdvice || "Siga os ensinamentos do Odu."}</p>
                                    </div>
                                </div>
                            )}
                            {activeTab === 'orishas' && (
                                <div className="space-y-4">
                                    <div className="bg-amber-900/20 border border-amber-700/50 p-4 rounded-lg">
                                        <div className="flex items-center justify-between mb-1">
                                            <h4 className="text-amber-300 font-bold uppercase text-sm">Orixá Regente deste Odu</h4>
                                            <TextReader text={`${data.rulingOrishas}. ${data.spirituality}`} />
                                        </div>
                                        <p className="text-2xl font-serif text-white font-bold">{data.rulingOrishas}</p>
                                    </div>
                                    <p className="text-ifa-text leading-relaxed whitespace-pre-line">{data.spirituality || "Consulte o Itan para detalhes sobre o Orixá deste Odu."}</p>
                                </div>
                            )}
                            {activeTab === 'oriki' && (
                            ...
                            {activeTab === 'ori' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Ori — Cabeça Espiritual</span>
                                        <TextReader text={data.destinyAndOri} />
                                    </div>
                                    <p>{data.destinyAndOri}</p>
                                </div>
                            )}
                            {activeTab === 'dangers' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Perigos e Avisos</span>
                                        <TextReader text={data.dangers} />
                                    </div>
                                    <p>{data.dangers}</p>
                                </div>
                            )}
                            {activeTab === 'diet' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Dieta (Ewo)</span>
                                        <TextReader text={`Recomendado: ${data.diet.positive}. Proibido: ${data.diet.negative}`} />
                                    </div>
                                    <p className="text-green-400 mb-2">👍 {data.diet.positive}</p>
                                    <p className="text-red-400">🚫 {data.diet.negative}</p>
                                </div>
                            )}
                            {activeTab === 'clothing' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Roupas e Cores</span>
                                        <TextReader text={`Recomendado: ${data.clothing.positive}. Evitar: ${data.clothing.negative}`} />
                                    </div>
                                    <p className="text-green-400 mb-2">👍 {data.clothing.positive}</p>
                                    <p className="text-red-400">🚫 {data.clothing.negative}</p>
                                </div>
                            )}
                            {activeTab === 'obstacles' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Obstáculos e Inimigos</span>
                                        <TextReader text={data.obstaclesAndEnemies} />
                                    </div>
                                    <p>{data.obstaclesAndEnemies}</p>
                                </div>
                            )}
                            {activeTab === 'ancestry' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Ancestralidade (Egungun)</span>
                                        <TextReader text={data.ancestry} />
                                    </div>
                                    <p>{data.ancestry}</p>
                                </div>
                            )}
                            {activeTab === 'personality' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Personalidade (Iwa)</span>
                                        <TextReader text={data.personality} />
                                    </div>
                            ...
                            {activeTab === 'decisions' && (
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-ifa-neutral uppercase font-bold">Tomada de Decisões</span>
                                        <TextReader text={data.decisionMaking} />
                                    </div>
                                    <p>{data.decisionMaking}</p>
                                </div>
                            )}
                            ...
                            {activeTab === 'baths' && (
                                <div className="bg-green-900/10 p-6 rounded border border-green-700">
                                    <div className="flex items-center justify-between mb-2">
                                        <h4 className="text-green-400 font-bold text-xl">{data.herbalBaths.name || 'Banho de Ervas'}</h4>
                                        <TextReader text={`${data.herbalBaths.name}. ${data.herbalBaths.purpose}. Ingredientes: ${(data.herbalBaths.ingredients || []).join(', ')}. Preparo: ${data.herbalBaths.preparation}`} />
                                    </div>
            ...
            <div className="mt-16 flex flex-col items-center gap-6">
                ...
                <div className="w-full max-w-3xl mx-auto mt-12 space-y-6">
                    <div className="flex items-center gap-3 mb-2">
                        <MessageCircle size={20} className="text-ifa-gold" />
                        <h3 className="text-ifa-gold font-serif font-bold text-lg">Pergunta Específica ao Oráculo</h3>
                    </div>
                    ...
                    {chatHistory.map(msg => (
                        <div key={msg.id} className="space-y-3 animate-fade-in">
                            <div className="flex justify-end">
                                <div className="bg-ifa-wood text-white px-4 py-2 rounded-xl rounded-br-sm max-w-[80%] text-sm">{msg.question}</div>
                            </div>
                            <div className="bg-ifa-surface border border-ifa-border p-4 rounded-xl rounded-tl-sm">
                                <p className="text-sm leading-relaxed whitespace-pre-line">{msg.answer}</p>
                                {msg.shortSummary && <p className="text-xs text-ifa-neutral mt-2 italic border-t border-ifa-border pt-2">{msg.shortSummary}</p>}
                                {(msg.basic || msg.medium || msg.complete) && <RitualPanel msg={msg} />}
                            </div>
                        </div>
                    ))}

                    {/* Campo de nova pergunta */}
                    <form onSubmit={handleAskQuestion} className="flex gap-2 bg-ifa-base-dark border border-ifa-border rounded-xl p-3">
                        <input
                            value={currentQuestion}
                            onChange={e => setCurrentQuestion(e.target.value)}
                            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleAskQuestion(); } }}
                            placeholder={chatHistory.length === 0 ? `Faça uma pergunta específica sobre ${data.oduName}...` : "Próxima pergunta..."}
                            disabled={isAsking}
                            className="flex-grow bg-transparent outline-none text-sm text-ifa-text placeholder-ifa-neutral/50"
                        />
                        <button type="submit" disabled={!currentQuestion.trim() || isAsking}
                            className="bg-ifa-gold text-ifa-base p-2 rounded-lg hover:opacity-90 disabled:opacity-40 transition-opacity flex items-center">
                ...
                <div className="w-full border-t border-ifa-border mt-8 pt-8 flex justify-center">
                    <button onClick={() => { onEndSession(selections, notes); setShowPrintCenter(true); }} className="px-12 py-4 bg-ifa-gold/10 border border-ifa-gold text-ifa-gold font-bold uppercase tracking-widest hover:bg-ifa-gold hover:text-black transition-colors rounded shadow-2xl flex items-center gap-2"><Printer size={20} /> {t.endSession}</button>
                </div>
            </div>
