...
function somSemente(delay: number) {
  setTimeout(() => {
    try {
...
const OpeleSeed: React.FC<OpeleSeedProps> = ({ state, onClick, position, isAnimating = false }) => {
  ...
  useEffect(() => {
    ...
    if (isAnimating) {
      somSemente(position * 70);
    }
  }, []);

  const rotFinal = useRef(`${(Math.random() * 16) - 8}deg`);

  return (
    <div className={`relative flex flex-col items-center ${isAnimating ? 'semente' : ''}`}
      style={isAnimating ? { '--i': position, '--rot-final': rotFinal.current } as any : {}}>
      ...
      <div
        ...
        className={`
          ...
          hover:brightness-110 hover:drop-shadow-[0_4px_8px_rgba(210,165,40,0.3)]
